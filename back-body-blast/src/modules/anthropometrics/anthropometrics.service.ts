...
@Injectable()
export class AnthropometricsService implements OnModuleInit {
  constructor(
    ...
    if (users.length > 0 && data.length > 0) {
      ...
      for (let i = 0; i < days; ++i) {
        ...
        const createdAnthropometrics = users.flatMap((user) => {
          ...
          if (
            Math.ceil(Math.abs(anthrpCreatedAt - createdDate.getTime()) / PeriodTime.dayTime) >= user.anthrpJobPeriod!
          ) {
  ...
  async findAll(request: GetAnthropometricsForUserRequest): Promise<AppDatePagination.Response<AnthropometricsEntity>> {
