...
import { Get, Query, Param, Req } from '@nestjs/common/decorators';
...
export class AdminAnthropometricsController {
  ...
  async getAll(@Req() req: RequestWithUser, @Query() query: GetAnthropometricsForUserByAdminRequest) {
    ...
    if (!ability.can(Action.Manage, AnthropometricsEntity)) throw MainException.forbidden('Cannot get anthropometrics');
