...
describe('AdminNutritionService', () => {
  ...
  describe('update method', () => {
    it('it should not update existing nutrition record because of wrong id', async () => {
      ...
      expect(service.update(savedData.id + 5, updateRequest)).rejects.toThrow();
    });
  });

  describe('update method', () => {
    it('it should not update existing nutrition record because of wrong data', async () => {
      const userRequest: CreateUserByAdminRequest = {
        email: 'test1@mail.ru',
        password: 'Qwertyuiop1',
        firstName: 'Test',
      ...
      const invalidRequest: UpdateNutritionByAdminRequest = {
        name: '',
      };

      const savedData = await repository.save(await repository.create(request));

      expect(service.update(savedData.id, invalidRequest)).rejects.toThrow();
    });
  });
});
