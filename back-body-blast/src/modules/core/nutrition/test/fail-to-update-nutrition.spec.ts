...
describe('BaseNutritionService', () => {
  ...
  describe('update method', () => {
    it('it should not update nutrition record because of wrong id', async () => {
      ...
      const invalidRequest: UpdateNutritionRequest = {
        name: 'dinner',
      };
      await expect(service.update(savedData.id + 5, invalidRequest)).rejects.toThrow();
    });
  });

  describe('update method', () => {
    it('it should not update nutrition record because of wrong data', async () => {
      const userRequest: CreateUserRequest = {
        email: 'test1@mail.ru',
        password: 'Qwertyuiop1',
        firstName: 'Test',
      ...
      await expect(service.update(savedData.id, invalidRequest)).rejects.toThrow();
    });
  });
});
